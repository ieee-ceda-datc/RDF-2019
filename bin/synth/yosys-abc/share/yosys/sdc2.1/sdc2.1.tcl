################################################################################
#
#  Copyright 2017(c) Synopsys, Inc.
#
################################################################################
#
# Title        : SDC-parser commands declaration for SDC version 2.1
#
...
#sdc1.8
 ...
 declare set_timing_derate {
  {derate_value        Float  {$par>=-1.0 && $par<=2.0 } }
  {object_list          List      }
# {derate_value         Float {$par>=0 && $par<=2.0}}
  {-min                 Flag      }
  {-max                 Flag      }
  {-rise                Flag      }
  {-fall                Flag      }
...
#sdc2.1 sdc 2.1 constructs
 declare set_sense {
 {-type      Enum  {clock data}   }
 {-non_unate		   Flag    }
  {-clocks                  List              }
  {-positive                Flag      }
  {-negative                Flag        }
  ...
  {object_list                     List  {1}    }
   ...
   (param(-positive) ^ param(-negative) ^ param(-pulse) ^ param(-stop_propagation) ^ (param(-non_unate) && param(-clocks)))}
# \ (param(-non_unate) && (param(-clock)}
